...
- 重複しているルールの優先度はどう処理されているか
  - 例えばno-debuggerは `@nuxtjs/eslint-config` と `eslint-config-standard` の両方で設定されているがどちらの設定が適用されるのか
  - rulesはextendsで適用されるルールを上書きしているイメージなのでこの例で言えば `@nuxtjs/eslint-config` のno-debuggerが優先される？
